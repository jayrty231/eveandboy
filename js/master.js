...
var index = $('.product_image_area').attr('id');

//product img
function prodImg(pickerVal){
    img = document.getElementById('productImg')
    if (typeof(products[index].productImg) == 'object' && pickerVal == '0') {
        selectedImg = products[index].productImg[1]
    }else if (typeof(products[index].productImg) != 'object'){
        selectedImg = products[index].productImg
    }else{
        selectedImg = products[index].productImg[0]
    }
    img.innerHTML = '<img class="img-fluid" src="../img/products/' + selectedImg + '" alt="">';
}

//build product detail
function buildProduct() {
    document.getElementById('productBrand').innerHTML = products[index].productBrand;
    document.getElementById('productName').innerHTML = products[index].productName;
    document.getElementById('productDes').innerHTML = products[index].productDes;
    document.getElementById('productNote').innerHTML = '<a><span>Notes</span>: </a>' + products[index].productNotes;
    document.getElementById('productPrice').innerHTML = products[index].productPrice[0].price;
    
    var scent = document.getElementById('productScent');
    scent.innerHTML = '<a><span>Scents</span>: </a>'
    for (var i = 0; i < products[index].productScent.length; i++) {
        if (products[index].productScent.length - i != 1){
            scent.innerHTML += products[index].productScent[i] + ', '
        }else{
            scent.innerHTML += products[index].productScent[i]
        }
    }
...
//build selectpicker
var id = document.getElementById('volAndPrice');
for (var i = 0; i < products[index].productPrice.length; i++) {
    id.innerHTML += '<option value="' + i + '">' + products[index].productPrice[i].vol + '</option>';
}

//get price from selectpicker
$('.selectpicker').change(function (){
    var selectedIndex = $(this).find('option:selected').val();
    prodImg(selectedIndex);
    var price = document.getElementById('productPrice');
    var selectedText = products[index].productPrice[selectedIndex].price;

    price.innerHTML = selectedText;
});
...
function setDafault(){
    ...
    $('select[id=volAndPrice]').val(val);
    document.getElementById('productPrice').innerHTML = products[index].productPrice[val].price;
}
                ...
                <div class="collapse navbar-collapse offset" id="navbarSupportedContent">\
                    <ul class="nav navbar-nav menu_nav ml-auto">\
                        ...
                        <li class="nav-item submenu dropdown">\
                            ...
                            <ul class="dropdown-menu">\
                                <li class="nav-item"><a class="nav-link" href="../price/price0.html">Less than $50</a></li>\
                                <li class="nav-item"><a class="nav-link" href="../price/price1.html">$50 - $100</a></li>\
                                <li class="nav-item"><a class="nav-link" href="../price/price2.html">$100 - $150</a></li>\
                                <li class="nav-item"><a class="nav-link" href="../price/price3.html">More than $150</a></li>\
                            </ul>\
                        </li>\
                    </ul>\
